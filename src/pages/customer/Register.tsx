...
export const Register: React.FC = () => {
  ...
  const getStrengthColor = () => {
    if (password.length === 0) return 'border-slate-300 bg-slate-100';
    if (password.length < 8 || !/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(password)) return 'border-red-300 bg-red-100';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // AuthContext will handle redirect after successful registration
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-50 via-white to-emerald-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400">Join Craftella to start creating unique experiences</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              <Input
                ...
                placeholder=" "
                className={`${errors.password ? 'border-red-500' : getStrengthColor()}`}
              />
              <label
                htmlFor="password"
                className="absolute left-3 top-2 text-slate-500 dark:text-slate-400 text-sm transition-all duration-300 peer-placeholder-shown:top-2 peer-placeholder-shown:text-sm peer-focus:top-2 peer-focus:text-sm peer-focus:text-primary"
              ...
              {password && errors.password && !errors.password.includes('match') && (
